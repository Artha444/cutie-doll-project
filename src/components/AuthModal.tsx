...
export default function AuthModal({
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    if (!emailRegex.test(email.trim())) {
      setErrorMsg('Format alamat email tidak sesuai (contoh: nama@email.com).');
      return;
    }
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2.5">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 py-3 px-4 bg-white border border-slate-200 rounded-2xl hover:bg-slate-50 transition-all shadow-sm hover:shadow-md cursor-pointer font-sans font-bold text-xs sm:text-sm text-slate-700"
                >
              ...
              <form onSubmit={handleAuthSubmit} className="space-y-4" noValidate>
                ...
                {process.env.NEXT_PUBLIC_HCAPTCHA_SITEKEY && (
                  <div className="flex justify-center w-full overflow-hidden py-1.5 animate-in fade-in duration-200">
                    <HCaptcha
                      ref={captchaRef}
                ...
                <button
                  ...
                  className="w-full py-3 bg-pink-500 hover:bg-pink-600 text-white rounded-2xl font-black text-xs sm:text-sm transition-all shadow-md shadow-pink-200 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center mt-2"
                >
                  {isLoading ? (
              ...
              <p className="text-[11px] sm:text-xs text-slate-500 text-center leading-relaxed font-medium mt-4">
            ...
            <div className="space-y-4">
              ...
              <div className="text-center space-y-2 mt-4 pt-2 border-t border-slate-100">
                <p className="text-[11px] sm:text-xs text-slate-500">
                  Tidak melihat email di kotak masuk?{' '}
                  {resendCountdown > 0 ? (
                    <span className="text-slate-400 font-bold">Kirim ulang ({resendCountdown}s)</span>
                  ) : (
                    <button
                      type="button"
